  ...
  useEffect(() => {
    const loadProfileData = async () => {
      ...
      if (user) {
        ...
        const count = getFollowerCount(user.id);
        setFollowerCount(await count);

        const listings = await getSellerListings(user.id);
        setUserListings(listings || []);

        const buyer = await getBuyerOrders(user.id);
        const seller = await getSellerOrders(user.id);
    ...
    >
      ...
      <View style={styles.section}>
        ...
        {userListings.length === 0 ? (
          ...
          >
            {userListings.length > 0 && userListings.slice(0, 3).map((listing) => (
              <TouchableOpacity
                key={listing.id}
      ...
      <View style={styles.section}>
        ...
        {buyerOrders.length === 0 ? (
          ...
          <View>
            {buyerOrders.length > 0 && buyerOrders.slice(0, 2).map((order) => (
              <TouchableOpacity
                key={order.id}
