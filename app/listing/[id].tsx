...
import { DeliveryMethod, FoodListing, PaymentMethod } from '@/types';
...
export default function ListingDetailScreen() {
  ...
  const [listing, setListing] = useState<FoodListing | null | undefined>(null);


useEffect(() => {
  const fetchListing = async () => {
    try {
      const data = await getListingById(id); // waits for actual value
      setListing(data); // ✅ sets resolved data
    } catch (err) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // ✅ Refresh listing from backend
      const updated = await getListingById(listing.id);
      setListing(updated ?? null); // use fallback in case of undefined
