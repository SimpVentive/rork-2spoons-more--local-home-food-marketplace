...
import { Order, FoodListing, Complaint, AdminDashboardData } from '@/types';
...
export default function AdminDashboard() {
  ...
  const loadAdminDashboardData = async () => {
    try {
      ...
      setDashboardData(dashboardData); // or useStore(dashboard).setData(response.data);
    } catch (error) {
  ...
  const loadStats = () => {
    ...
    const totalChefs = dashboardData?.topChefs.length ?? 0;
    const verifiedChefs = dashboardData?.topChefs.filter(chef => chef.isVerified).length ?? 0;
    ...
    setStats({
      totalUsers: (dashboardData?.totalBuyers ?? 0) + (dashboardData?.topChefs?.length ?? 0),
      ...
      newListingsToday,
      newComplaintsToday,
      totalChefs: dashboardData?.topChefs?.length ?? 0,
      verifiedChefs: dashboardData?.topChefs?.filter(chef => chef.isVerified).length ?? 0,
      pendingChefs:
        (dashboardData?.topChefs?.length ?? 0) -
        (dashboardData?.topChefs?.filter(chef => chef.isVerified).length ?? 0),
      averageRating,
      totalBuyers: dashboardData?.totalBuyers ?? 0,
