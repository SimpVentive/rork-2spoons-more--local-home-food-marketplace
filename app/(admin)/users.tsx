...
import { 
  Search, User, ChefHat, Phone, Mail, MapPin,
  Star, ShoppingBag, MoreVertical, Ban, Edit,
  Trash, Shield 
...
export default function ManageUsers() {
  ...
  async function loadUsers() {
    setIsLoading(true);
    try {
      const res = await api.get('api/admin/users/'); // 🛠️ Set your Django API endpoint
      setUsers(res.data);
  ...
  function filterUsers() {
    ...
    if (filter === 'buyers') result = result.filter(u => !u.isChef && !u.isAdmin);
    else if (filter === 'chefs') result = result.filter(u => u.isChef);
    else if (filter === 'admins') result = result.filter(u => u.isAdmin);
  ...
  function handleUserAction(user: UserType, action: 'view' | 'edit' | 'ban' | 'delete') {
    setShowActionMenu(null);

    if (action === 'view') router.push(`/admin/user-details/${user.id}`);
    else if (action === 'edit') router.push(`/admin/edit-user/${user.id}`);
    else if (action === 'ban') {
      ...
      Alert.alert('Delete User', `Delete ${user.name}? This cannot be undone.`, [
        ...
        { text: 'Delete', style: 'destructive', onPress: () => {
          ...
          Alert.alert('User Deleted', `${user.name} was deleted.`);
        }}
      ]);
    }
  }

  function toggleVerification(userId: string) {
    setUsers(users.map(u => u.id === userId ? { ...u, isVerified: !u.isVerified } : u));
  }

  function renderUserItem({ item }: { item: UserType }) {
    return (
      <TouchableOpacity style={styles.userCard} onPress={() => handleUserAction(item, 'view')}>
        <Image
        ...
        <View style={styles.userActions}>
          {showActionMenu === item.id ? (
            <View style={styles.actionMenu}>
              <TouchableOpacity style={styles.actionMenuItem} onPress={() => handleUserAction(item, 'edit')}>
                <Edit size={16} color={colors.text} />
                <Text style={styles.actionMenuItemText}>Edit</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.actionMenuItem} onPress={() => handleUserAction(item, 'ban')}>
                <Ban size={16} color={colors.error} />
                <Text style={[styles.actionMenuItemText, { color: colors.error }]}>Ban</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.actionMenuItem} onPress={() => handleUserAction(item, 'delete')}>
                <Trash size={16} color={colors.error} />
                <Text style={[styles.actionMenuItemText, { color: colors.error }]}>Delete</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <TouchableOpacity style={styles.actionButton} onPress={() => setShowActionMenu(item.id)}>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.filterContainer}>
        {['all', 'buyers', 'chefs', 'admins'].map(type => (
          <TouchableOpacity
            key={type}
      ...
      <TouchableOpacity style={styles.addButton} onPress={() => router.push('/admin/add-user')}>
