...
export default function ManageOrders() {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Unable to fetch orders from server.');
    }
    setIsLoading(false);
  };
  ...
  const handleOrderAction = (order: Order, action: 'view' | 'complete' | 'refund') => {
    if (action === 'view') {
      ...
      Alert.alert("Complete Order", `Mark order #${order.id} as completed?`, [
        { text: "Cancel", style: "cancel" },
        { 
          text: "Complete", 
          onPress: () => {
            const updated = orders.map(item => item.id === order.id 
              ? { ...item, status: 'completed', updatedAt: new Date().toISOString(), completedAt: new Date().toISOString() }
              : item
            );
            setOrders(updated);
            Alert.alert("Order Completed", `Order #${order.id} is completed.`);
          }
        }
      ]);
    } else if (action === 'refund') {
      Alert.alert("Refund Order", `Refund order #${order.id}?`, [
        { text: "Cancel", style: "cancel" },
        { 
          text: "Refund", style: "destructive",
          onPress: () => {
            const updated = orders.map(item => item.id === order.id 
              ? { ...item, status: 'refunded', paymentStatus: 'refunded', updatedAt: new Date().toISOString() }
              : item
            );
            setOrders(updated);
  ...
  const getStatusColor = (status: OrderStatus) => {
    switch (status) {
      ...
      case 'refund_requested': return '#FF9800';
  ...
  const getStatusIcon = (status: OrderStatus) => {
    switch (status) {
      case 'completed': return <CheckCircle size={16} color="#43A047" />;
      case 'canceled':
      case 'refunded': return <XCircle size={16} color="#E53935" />;
      case 'refund_requested': return <RefreshCw size={16} color="#FF9800" />;
      default: return <Clock size={16} color="#1976D2" />;
    }
  };

  const renderOrderItem = ({ item }: { item: Order }) => (
    <TouchableOpacity style={styles.orderCard} onPress={() => handleOrderAction(item, 'view')}>
      <Image source={{ uri: item.listingSnapshot?.image }} style={styles.orderImage} contentFit="cover" />
      ...
      <View style={styles.orderActions}>
        {['completed', 'refunded', 'canceled'].includes(item.status) ? null : (
          <TouchableOpacity 
        ...
        {['refunded', 'canceled'].includes(item.status) ? null : (
          <TouchableOpacity 
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.filterContainer}>
        {['all', 'pending', 'completed', 'canceled'].map(type => (
          <TouchableOpacity
            key={type}
