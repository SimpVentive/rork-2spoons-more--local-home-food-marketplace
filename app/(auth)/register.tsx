...
export default function RegisterScreen() {
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets?.length > 0) {
        ...
        requestAnimationFrame(() => {
          updateFormData('profileImage', uri);
        });
      }
    } catch (error) {
  ...
  const toggleCuisineType = (cuisine: string) => {
    ...
    index === -1 ? cuisineTypes.push(cuisine) : cuisineTypes.splice(index, 1);
  ...
  const togglePaymentMethod = (method: string) => {
    ...
    index === -1 ? paymentMethods.push(method) : paymentMethods.splice(index, 1);
  ...
  const getTagStyle = (selected: boolean) => selected ? [styles.tag, styles.selectedTag] : styles.tag;
  const getTagTextStyle = (selected: boolean) => selected ? [styles.tagText, styles.selectedTagText] : styles.tagText;
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>{step === 1 ? 'Basic Information' : 'Seller Profile'}</Text>
        </View>
        ...
        {step === 1 ? (
          <View style={styles.form}>
            <Input label="Full Name" placeholder="Enter your full name" value={formData.name}
              onChangeText={(text) => updateFormData('name', text)} error={errors.name} />

            <Input label="Email" placeholder="Enter your email" value={formData.email}
              onChangeText={(text) => updateFormData('email', text)} keyboardType="email-address"
              autoCapitalize="none" error={errors.email} />

            <Input label="Password" placeholder="Create a password" value={formData.password}
              onChangeText={(text) => updateFormData('password', text)} isPassword
              error={errors.password} secureTextEntry={true} />
            ...
            <View style={styles.sectionContainer}>
              ...
              <View style={styles.tagsContainer}>
                {CUISINE_TYPES.map((cuisine) => (
                  <TouchableOpacity
                    key={cuisine}
                    style={getTagStyle(formData.cuisineTypes.includes(cuisine))}
            ...
            <View style={styles.sectionContainer}>
              ...
              <View style={styles.tagsContainer}>
                {PAYMENT_METHODS.map((method) => (
                  <TouchableOpacity
                    key={method}
                    style={getTagStyle(formData.paymentMethods.includes(method))}
        ...
        <View style={styles.buttonContainer}>
          {step > 1 && <Button title="Back" onPress={handleBack} variant="outline" />}
          <Button title={step === 1 ? 'Next' : 'Create Account'} onPress={handleNext} isLoading={isLoading} />
        </View>
...
// NOTE: Your `styles` should still be defined at the bottom or imported
