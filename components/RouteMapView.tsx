...
let RouteMapViewNative: React.ComponentType<RouteMapViewProps> | null = null;

// Only import native component on native platforms
if (Platform.OS !== 'web') {
  try {
    RouteMapViewNative = require('./RouteMapViewNative').default;
  } catch (error) {
    console.warn('Failed to load RouteMapViewNative:', error);
  }
}
...
export default function RouteMapView({ routePoints, dishesOnRoute, onDishPress }: RouteMapViewProps) {
  ...
  // For native platforms, use the native map component if available
  if (Platform.OS !== 'web' && RouteMapViewNative) {
    return (
      <RouteMapViewNative
  ...
  // For web and fallback, show a comprehensive route overview
  return (
      ...
      <View style={styles.mapNotice}>
        <Text style={styles.mapNoticeText}>
          {Platform.OS === 'web' 
            ...
            : '🗺️ Map view temporarily unavailable'
          }
        </Text>
      </View>
    </View>
